...
describe('API interceptors', () => {
  ...
  it('invokes unauthorized handler only once for repeated 401s', async () => {
    ...
    const firstRejection = responseHandlers[0].rejected(error).catch((err: any) => err)
    const secondRejection = responseHandlers[0].rejected(error).catch((err: any) => err)
  ...
  it('normalizes validation errors', async () => {
    ...
    await expect(responseHandlers[0].rejected(error)).rejects.toMatchObject({
      appError: {
        code: 'VALIDATION_ERROR',
        validationErrors: [{ field: 'username', message: 'Username is required' }],
      },
    })
