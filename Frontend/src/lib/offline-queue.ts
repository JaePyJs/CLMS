...
class OfflineQueue {
  ...
  private async processQueueItem(item: QueueItem) {
    if (item.type === 'api-call' && item.endpoint && item.method) {
      ...
      const response = await apiClient.post('/api/students/activity', item.data)
      return response
  ...
  // Get cached data
  async getCachedData(store: string, maxAge: number = 5 * 60 * 1000): Promise<any[]> {
    const db = await this.init()
    const allData = await db.getAll(store)
    const now = Date.now()

    return allData.filter(item =>
      item._cached && (now - item._cacheTime) < maxAge
    )
  }
...
export const offlineActions = {
  ...
  startSession: (equipmentId: string, studentId: string, timeLimitMinutes: number) => {
    return offlineQueue.addToQueue({
      type: 'session-action',
  ...
  apiCall: (endpoint: string, method: 'GET' | 'POST' | 'PUT' | 'DELETE', data?: any) => {
    return offlineQueue.addToQueue({
      type: 'api-call',
      endpoint,
      method,
