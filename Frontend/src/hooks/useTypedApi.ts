...
export function useTypedApi<T>(
  ...
  const execute = useCallback(async (customConfig?: typeof config) => {
  ...
  }, [config, transform, validate, onSuccess, onError, retryCount, retryDelay, state.retryCount]);
...
export function useTypedMutation<TRequest, TResponse>(
  ...
  const { onSuccess, onError, onMutate, optimisticUpdate, rollbackOnError } = options;

  const mutate = useCallback(async (requestData: TRequest) => {
    setLoading(true);
    setError(null);
 ...
 */
export function useTypedRealTime<T>(
  config: {
    url: string;
    interval?: number;
  }
