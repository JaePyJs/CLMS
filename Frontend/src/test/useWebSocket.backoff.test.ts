...
describe('useWebSocket exponential backoff', () => {
  const Wrapper: React.FC<{ children: React.ReactNode }> = ({ children }) => <TestProviders>{children}</TestProviders>
  it('initializes hook state and exposes API', () => {
    const { result } = renderHook(() => useWebSocket({ autoConnect: false }), { wrapper: Wrapper })
