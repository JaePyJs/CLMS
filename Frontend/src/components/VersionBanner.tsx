...
export default function VersionBanner() {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        const resp = await fetch(`${apiBase}/api/version`, {
          cache: 'no-store',
        });
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-center px-4 py-2 bg-yellow-500 text-black">
      <span className="text-sm">
        New version available (backend {backendVersion}, frontend{' '}
        {frontendVersion}). Refresh to update.
      </span>
      <button
        className="ml-3 px-2 py-1 bg-black text-yellow-300 text-xs rounded"
