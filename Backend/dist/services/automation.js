...
class AutomationService {
    ...
    async initialize() {
        ...
        try {
            ...
            logger_1.logger.error('Failed to initialize automation service', { error: error.message });
            throw error;
        }
    }
    ...
    async loadScheduledJobs() {
        try {
            ...
            logger_1.logger.error('Failed to load scheduled jobs', { error: error.message });
        }
    }
    async scheduleJob(job) {
        try {
            ...
            logger_1.logger.error(`Failed to schedule job: ${job.name}`, { error: error.message });
        }
    }
    async executeJob(job) {
        const startTime = logger_1.performanceLogger.start('job_execution', { jobName: job.name });
        try {
            await this.prisma.automationJob.update({
                where: { id: job.id },
                data: { status: client_1.JobStatus.RUNNING, lastRunAt: new Date() }
            });
            logger_1.automationLogger.jobStart(job.name, job.id, typeof job.config === 'string' ? JSON.parse(job.config) : job.config || {});
            let result;
            switch (job.type) {
            ...
            logger_1.performanceLogger.end('job_execution', startTime, { jobName: job.name, success: result.success });
            ...
            logger_1.performanceLogger.end('job_execution', startTime, { jobName: job.name, success: false });
    ...
    async processGoogleSheetsSync(job) {
        const startTime = Date.now();
        const { activities, jobName, jobId } = job.data;
        try {
            const result = await googleSheets_1.googleSheetsService.syncStudentActivities(activities);
            if (result.success) {
                await this.prisma.activity.updateMany({
                    where: {
                        id: { in: activities.map((a) => a.id) }
                    },
                    data: { googleSynced: true }
                });
            }
            await googleSheets_1.googleSheetsService.logAutomationTask(jobName, 'SYNC', result.success ? 'COMPLETED' : 'FAILED', {
                ...
                recordsProcessed: result.rowsSynced,
                triggeredBy: 'SCHEDULED',
            ...
            if (result.errors.length > 0) {
                jobResult.errorMessage = result.errors.join('; ');
            }
            return jobResult;
        }
        catch (error) {
    ...
    async logJobExecution(jobId, status, result) {
        try {
            ...
            logger_1.logger.error('Failed to log job execution', { error: error.message, jobId, status });
        }
    }
    getNextRunTime(cronExpression) {
        try {
            const task = node_cron_1.default.schedule(cronExpression, () => { }, { scheduled: false });
    ...
    setupCleanupInterval() {
        setInterval(async () => {
            try {
                ...
                logger_1.logger.error('Failed to cleanup old automation logs', { error: error.message });
            }
        }, 60 * 60 * 1000);
    }
    async triggerJob(jobId, userId = 'MANUAL') {
        try {
            ...
            logger_1.logger.error('Failed to trigger job', { jobId, error: error.message });
            throw error;
        }
    }
    async getJobStatus(jobId) {
        try {
            ...
            logger_1.logger.error('Failed to get job status', { jobId, error: error.message });
            throw error;
        }
    }
    async getAllJobs() {
        try {
            ...
            logger_1.logger.error('Failed to get all jobs', { error: error.message });
            throw error;
        }
    }
