...
exports.generateSampleData = main;
...
async function generateBooks() {
    ...
    for (let i = 1; i <= CONFIG.books; i++) {
        const title = faker_1.faker.lorem.words({ min: 3, max: 8 }).split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
...
async function generateActivities(students, equipment) {
...
async function generateAutomationJobs() {
    ...
    for (const job of jobs) {
        await prisma.automationJob.upsert({
            ...
            create: {
                ...job,
                nextRunAt: new Date(Date.now() + getRandomInt(60000, 86400000))
            }
        });
    }
