...
async function importStudentsFromJSON() {
    try {
        ...
        const jsonPath = (0, path_1.join)(process.cwd(), '..', 'Current_Students.json');
...
async function importBooksFromExcel() {
    try {
        ...
        const excelPath = (0, path_1.join)(process.cwd(), '..', 'SHJCS_Complete_Library_Database_Organized.xlsx');
        ...
        worksheet.eachRow({ includeEmpty: false }, async (row, rowNumber) => {
            if (rowNumber === 1)
                return;
            try {
                const accessionNo = row.getCell(1).value?.toString() || `BK-${Date.now()}-${rowNumber}`;
...
async function main() {
    ...
    try {
        ...
        console.log('\n📊 Import Summary:');
        console.log(`  Students: ${studentsResult.imported} imported, ${studentsResult.skipped} skipped`);
        console.log(`  Books: ${booksResult.imported} imported, ${booksResult.skipped} skipped`);
