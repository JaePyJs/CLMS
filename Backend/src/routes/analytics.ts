  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        statistics: {
          overdue_rate: activeBorrows > 0 ? (overdueBorrows / activeBorrows * 100).toFixed(2) : 0,
          return_rate: (returnedBorrows / (activeBorrows + returnedBorrows) * 100).toFixed(2) || 0
        }
      }
    });
  } catch (error) {
    logger.error('Error retrieving dashboard analytics', { error: error instanceof Error ? error.message : 'Unknown error' });
    throw error;
  }
}));

// GET /api/analytics/students - Student statistics
router.get('/students', authenticate, asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({
      success: true,
      data: {
        total_students: await prisma.students.count({ where: { is_active: true } }),
    ...
    logger.error('Error retrieving student analytics', { error: error instanceof Error ? error.message : 'Unknown error' });
    throw error;
  }
}));

// GET /api/analytics/books - Book statistics
router.get('/books', authenticate, asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const totalBooks = await prisma.books.count({ where: { is_active: true } });
    ...
    res.json({
      ...
      data: {
        ...
        top_books: topBooks,
        availability_rate: totalBooks > 0 ? (availableBooks / totalBooks * 100).toFixed(2) : 0,
        period_days: days
      }
    });
  } catch (error) {
    logger.error('Error retrieving book analytics', { error: error instanceof Error ? error.message : 'Unknown error' });
    throw error;
  }
}));

// GET /api/analytics/borrows - Borrowing statistics
router.get('/borrows', authenticate, asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.error('Error retrieving borrow analytics', { error: error instanceof Error ? error.message : 'Unknown error' });
    throw error;
  }
}));

// GET /api/analytics/equipment - Equipment statistics
router.get('/equipment', authenticate, asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({
      success: true,
      data: {
        total_equipment: await prisma.equipment.count({ where: { is_active: true } }),
    ...
    logger.error('Error retrieving equipment analytics', { error: error instanceof Error ? error.message : 'Unknown error' });
    throw error;
  }
}));
