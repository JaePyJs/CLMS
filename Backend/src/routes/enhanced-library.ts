...
function calculateFine(grade_level: string, overdueDays: number): number {
  ...
  if (grade_level.includes('Grade 1') || grade_level.includes('Grade 2') || grade_level.includes('Grade 3')) {
    rate = GRADE_FINE_RATES['Primary'];
  } else if (grade_level.includes('Grade 4') || grade_level.includes('Grade 5') || grade_level.includes('Grade 6')) {
...
function getDueDate(material_type: string): Date {
  const policy = MATERIAL_POLICIES[material_type as keyof typeof MATERIAL_POLICIES];
...
router.get('/user-tracking', authenticateToken, async (_req: Request, res: Response) => {
  try {
...
router.post('/borrow', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const book = await prisma.books.findUnique({
      where: { id: bookId },
      select: { id: true, title: true, available_copies: true, is_active: true }
    });

    if (!book || (book.available_copies ?? 0) <= 0 || book.is_active === false) {
      return res.status(400).json({
        success: false,
    ...
    res.json({
      ...
      data: {
        checkout,
        dueDate,
        finePerDay: MATERIAL_POLICIES[material_type as keyof typeof MATERIAL_POLICIES].finePerDay,
      },
    });
  } catch (error) {
    if (isDevelopment()) {
      ...
      return res.json({
        ...
        data: {
          ...
          dueDate,
          finePerDay: MATERIAL_POLICIES[String(material_type || 'Fiction') as keyof typeof MATERIAL_POLICIES]?.finePerDay || 5,
        },
      });
    }
...
router.post('/return', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const now = new Date();
    const dueDate = new Date(checkout.due_date as Date);
    const overdueDays = Math.max(0, Math.ceil((now.getTime() - dueDate.getTime()) / 86400000));

    // Calculate fine based on grade level
    const fineAmount = calculateFine(`Grade ${checkout.student.grade_level}`, overdueDays);
...
router.get('/overdue', authenticateToken, async (_req: Request, res: Response) => {
  try {
    const now = new Date();
    ...
    const overdueItems = overdueCheckouts.map(checkout => {
      const overdueDays = Math.ceil((now.getTime() - new Date(checkout.due_date as Date).getTime()) / 86400000);
    ...
    res.json({
      ...
      data: {
        ...
        totalFines: overdueItems.reduce((sum, item) => sum + item.fineAmount, 0),
        items: overdueItems,
      },
    });
  } catch (error) {
    ...
    if (isDevelopment()) {
      ...
      return res.json({
        ...
        data: {
          ...
          totalFines: fallbackItems.reduce((sum, item) => sum + item.fineAmount, 0),
          items: fallbackItems,
        },
      });
    }
...
// Top users analytics
router.get('/top-users', authenticateToken, async (req: Request, res: Response) => {
  try {
...
router.get('/books/available', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    if (material_type) whereClause.category = material_type;
    ...
    if (books.length === 0 && isDevelopment()) {
      const sample = [
        { id: 'BOOK-DEV-1', title: 'Sample Book', author: 'John Doe', isbn: '9780000000001', material_type: 'Fiction', status: 'AVAILABLE' },
        { id: 'BOOK-DEV-2', title: 'Heritage', author: 'Maria Santos', isbn: '9780000000002', material_type: 'Filipiniana', status: 'AVAILABLE' },
        { id: 'BOOK-DEV-3', title: 'ABCs', author: 'Baby Reader', isbn: '9780000000003', material_type: 'Easy Books', status: 'AVAILABLE' },
      ];
    ...
    res.status(500).json({ success: false, error: 'Failed to retrieve available books' });
  }
});

// Search books with material type filtering
router.get('/books/search', authenticateToken, async (req: Request, res: Response) => {
  try {
...
router.get('/borrowed/:student_id', authenticateToken, async (req: Request, res: Response) => {
  try {
...
router.get('/borrowed/by-accession/:barcode', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    if (!checkout) {
      return res.status(404).json({ success: false, error: 'No active checkout found' });
    }
    return res.json({ success: true, data: checkout });
  } catch (error) {
    res.status(500).json({ success: false, error: 'Failed to find active checkout' });
  }
});

// Calculate fine for a specific checkout
router.get('/fine/:checkoutId', authenticateToken, async (req: Request, res: Response) => {
  try {
...
router.post('/fine/:checkoutId/pay', authenticateToken, async (req: Request, res: Response) => {
  try {
    const { checkoutId } = req.params;
...
router.post('/overdue/reminder', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    //   },
    // });
    const notification = { id: 'placeholder', message: 'Notification system not implemented' };
...
router.get('/policies', authenticateToken, async (_req: Request, res: Response) => {
  try {
    res.json({
      success: true,
...
router.get('/grade-fines', authenticateToken, async (_req: Request, res: Response) => {
  try {
    res.json({
      success: true,
...
router.get('/reports/monthly', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const totalBorrowed = monthlyStats.find(s => s.status === 'ACTIVE')?._count || 0;
    const totalReturned = monthlyStats.find(s => s.status === 'RETURNED')?._count || 0;
    ...
    const topBorrowers = await Promise.all(
      topBorrowersAgg.map(async (row) => {
        const s = await prisma.students.findUnique({ where: { id: row.student_id }, select: { student_id: true, first_name: true, last_name: true, grade_level: true } });
        return { student_id: s?.student_id, name: `${s?.first_name ?? ''} ${s?.last_name ?? ''}`.trim(), grade_level: s?.grade_level, borrowCount: row._count.id };
      })
    );
...
// Export report
router.get('/reports/:reportId/export', authenticateToken, async (req: Request, res: Response) => {
  try {
...
// Get inventory
router.get('/inventory', authenticateToken, async (_req: Request, res: Response) => {
  try {
...
// Scan barcode
router.get('/inventory/scan/:barcode', authenticateToken, async (req: Request, res: Response) => {
  try {
    const { barcode } = req.params;
...
// Get print jobs
router.get('/printing/jobs', authenticateToken, async (_req: Request, res: Response) => {
  try {
...
// Create print job
router.post('/printing/jobs', authenticateToken, async (req: Request, res: Response) => {
  try {
    const { student_id, documentName, pages, copies, color, doubleSided } = req.body;
...
router.put('/printing/jobs/:_jobId/status', authenticateToken, async (req: Request, res: Response) => {
  try {
...
// Get print pricing
router.get('/printing/pricing', authenticateToken, async (_req: Request, res: Response) => { try {
    const pricing = {
