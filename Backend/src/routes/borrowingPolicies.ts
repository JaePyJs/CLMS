...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const policy = await BorrowingPolicyService.getPolicyByCategory(
      req.params['category'],
    );
    if (!policy) {
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!category || (loan_days === undefined && !overnight)) {
      res.status(400).json({
        success: false,
        message: 'category and loan_days or overnight required',
      });
      return;
    }
    const created = await BorrowingPolicyService.createPolicy({
      name,
      category,
      loan_days: loan_days ?? 0,
      overnight,
      is_active,
    });
...
router.put(
  ...
  authenticate,
  asyncHandler(async (req: Request, res: Response) => {
    const updated = await BorrowingPolicyService.updatePolicy(
      req.params['id'],
      req.body,
    );
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!checkoutDate || (!policyId && !category)) {
      res.status(400).json({
        success: false,
        message: 'checkoutDate and policyId or category required',
      });
      return;
    }
    let policy: any = null;
    if (policyId) {
      policy = (await BorrowingPolicyService.listPolicies(false)).find(
        p => p.id === policyId,
      );
    } else {
    ...
    if (!policy) {
      ...
      const fallback: Record<
        string,
        { loan_days: number; overnight: boolean }
      > = {
      ...
      const dueDate = BorrowingPolicyService.computeDueDate(
        new Date(checkoutDate),
        { loan_days: fb.loan_days, overnight: fb.overnight },
      );
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!bookId || !policyId) {
      res
        .status(400)
        .json({ success: false, message: 'bookId and policyId are required' });
      return;
    }
    const result = await BorrowingPolicyService.assignDefaultPolicyToBook(
      bookId,
      policyId,
    );
