...
router.post('/students', authenticate, asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        if (!studentData.student_id) {
          ...
          result.errors.push(`Row ${i + 1}: Missing required field 'student_id'`);
          continue;
        }

        if (!studentData.first_name) {
          result.skippedRecords++;
          result.errors.push(`Row ${i + 1}: Missing required field 'first_name'`);
          continue;
        }

        if (!studentData.last_name) {
          result.skippedRecords++;
          result.errors.push(`Row ${i + 1}: Missing required field 'last_name'`);
          continue;
        }
        ...
        if (existingStudent && !options.allowUpdates) {
          result.skippedRecords++;
          result.errors.push(`Row ${i + 1}: Student with ID '${studentData.student_id}' already exists`);
          continue;
        }
        ...
        const createData = {
          ...
          grade_level: studentData.grade_level ? parseInt(studentData.grade_level) : 1,
          email: studentData.email || null,
          barcode: studentData.barcode || null,
          grade_category: studentData.grade_category || null,
          is_active: studentData.is_active !== undefined ? studentData.is_active === true || studentData.is_active === 'true' : true
        };
    ...
    logger.error('Error importing students', { error: error instanceof Error ? error.message : 'Unknown error' });
    throw error;
  }
}));

// POST /api/import/books - Import books from CSV/Excel
router.post('/books', authenticate, asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        if (!bookData.accession_no) {
          result.skippedRecords++;
          result.errors.push(`Row ${i + 1}: Missing required field 'accession_no'`);
          continue;
        }

        if (!bookData.category) {
          result.skippedRecords++;
          result.errors.push(`Row ${i + 1}: Missing required field 'category'`);
          continue;
        }
        ...
        if (existingBook && !options.allowUpdates) {
          result.skippedRecords++;
          result.errors.push(`Row ${i + 1}: Book with accession number '${bookData.accession_no}' already exists`);
          continue;
        }
        ...
        const createData = {
          ...
          available_copies: bookData.available_copies ? parseInt(bookData.available_copies) : 1,
          total_copies: bookData.total_copies ? parseInt(bookData.total_copies) : 1,
          cost_price: bookData.cost_price ? parseFloat(bookData.cost_price) : null,
          ...
          is_active: bookData.is_active !== undefined ? bookData.is_active === true || bookData.is_active === 'true' : true
        };

        // Validate available vs total copies
        if (createData.available_copies > createData.total_copies) {
          result.skippedRecords++;
          result.errors.push(`Row ${i + 1}: Available copies cannot exceed total copies`);
          continue;
        }
    ...
    logger.error('Error importing books', { error: error instanceof Error ? error.message : 'Unknown error' });
    throw error;
  }
}));

// GET /api/import/template/:type - Download CSV template
router.get('/template/:type', authenticate, asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (type === 'students') {
      headers = ['student_id', 'first_name', 'last_name', 'grade_level', 'email', 'barcode', 'grade_category'];
      sampleData = [
        {
          student_id: 'STU001',
          first_name: 'John',
      ...
      headers = [
        'title', 'author', 'isbn', 'publisher', 'category', 'subcategory',
        'location', 'accession_no', 'available_copies', 'total_copies',
        'cost_price', 'edition', 'pages', 'remarks', 'source_of_fund',
        'volume', 'year'
      ];
      sampleData = [
        {
    ...
    logger.error('Error generating template', { error: error instanceof Error ? error.message : 'Unknown error' });
    throw error;
  }
}));

// POST /api/import/export/:type - Export data to CSV
router.post('/export/:type', authenticate, asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (type === 'students') {
      ...
      headers = ['student_id', 'first_name', 'last_name', 'grade_level', 'email', 'barcode', 'grade_category', 'is_active', 'created_at'];
    } else if (type === 'books') {
      ...
      headers = [
        ...
        'location', 'accession_no', 'available_copies', 'total_copies',
        'cost_price', 'edition', 'pages', 'remarks', 'source_of_fund',
        'volume', 'year', 'is_active', 'created_at'
      ];
      ...
      headers = [
        'student_id', 'student_name', 'book_title', 'book_author',
        'accession_no', 'checkout_date', 'due_date', 'return_date',
        'status', 'fine_amount'
      ];
    } else {
      return res.status(400).json({
    ...
    if (type === 'students') {
      ...
      rows = data.map(borrow => ({
        ...
        return_date: borrow.return_date ? borrow.return_date.toISOString() : '',
    ...
    logger.error('Error exporting data', { error: error instanceof Error ? error.message : 'Unknown error' });
    throw error;
  }
}));
