...
import { GradeCategory, EquipmentType, EquipmentStatus, ActivityType } from '@prisma/client'
...
export class ImportService {
    ...
    try {
      ...
      for (const record of records) {
        try {
          ...
          if (!Object.values(GradeCategory).includes(record.gradeCategory as GradeCategory)) {
          ...
          if (existingStudent) {
            logger.info(`Student already exists, skipping: ${record.studentId}`)
      ...
      logger.error('Failed to import students', { error: (error as Error).message })
  ...
  async importBooks(filePath: string): Promise<ImportResult> {
    ...
    try {
      ...
      logger.error('Failed to import books', { error: (error as Error).message })
      result.success = false
      result.errors.push((error as Error).message)
      return result
    }
    ...
    try {
      ...
      for (const record of records) {
        try {
          ...
          if (!Object.values(EquipmentType).includes(record.type as EquipmentType)) {
          ...
          if (existingEquipment) {
            logger.info(`Equipment already exists, skipping: ${record.equipmentId}`)
            result.skippedRecords++
            continue
          }
      ...
      logger.error('Failed to import equipment', { error: (error as Error).message })
      result.success = false
      result.errors.push((error as Error).message)
      return result
    }
