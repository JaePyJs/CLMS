...
test.describe("🎯 CLMS Admin - All Buttons Functional Test", () => {
  ...
  async function navigateToTab(page: any, tabName: string) {
    ...
    // Try desktop approach first - visible tabs
    try {
    ...
    // Try mobile approach - look for collapsed menu or hamburger menu
    try {
      ...
      for (const selector of mobileMenuSelectors) {
        try {
          ...
          console.log(`📱 Opened mobile menu with ${selector}`);
          
          // Now try to click the tab in the mobile menu
          await page.click(`[role="tab"]:has-text("${tabName}")`, { timeout: 5000 });
          console.log(`✅ Found ${tabName} tab in mobile menu`);
          return;
        } catch (e) {
          continue;
        }
      ...
      console.log(`⚠️ Could not open mobile menu for ${tabName}`);
    }
    
    // Final fallback - look for any element containing the tab name
    try {
      await page.click(`text=${tabName}`, { timeout: 5000 });
      console.log(`✅ Found ${tabName} using text selector`);
      return;
    } catch (error) {
